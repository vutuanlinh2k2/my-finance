 ...
 */
const yearlyExpenseTotals: Partial<
  Record<string, Record<string, Array<MonthlyTagTotal>>>
> = {
  '2026': {
    'tag-1': MONTH_NAMES.map((monthName, i) => ({
      month: i,
      monthName,
      amount: i === 0 ? 5500000 : 0,
    })),
    'tag-2': MONTH_NAMES.map((monthName, i) => ({
      month: i,
      monthName,
      amount: i === 0 ? 3200000 : 0,
    })),
    'tag-3': MONTH_NAMES.map((monthName, i) => ({
      month: i,
      monthName,
      amount: i === 0 ? 2800000 : 0,
    })),
    'tag-4': MONTH_NAMES.map((monthName, i) => ({
      month: i,
      monthName,
      amount: i === 0 ? 2000000 : 0,
    })),
    'tag-5': MONTH_NAMES.map((monthName, i) => ({
      month: i,
      monthName,
      amount: i === 0 ? 1500000 : 0,
    })),
  },
  '2025': {
    'tag-1': MONTH_NAMES.map((monthName, i) => ({
 ...
 */
const yearlyIncomeTotals: Partial<
  Record<string, Record<string, Array<MonthlyTagTotal>>>
> = {
  '2026': {
    'tag-10': MONTH_NAMES.map((monthName, i) => ({
      month: i,
      monthName,
      amount: i === 0 ? 25000000 : 0,
    })),
    'tag-11': MONTH_NAMES.map((monthName, i) => ({
      month: i,
      monthName,
      amount: i === 0 ? 8000000 : 0,
    })),
    'tag-12': MONTH_NAMES.map((monthName, i) => ({
      month: i,
      monthName,
      amount: i === 0 ? 2800000 : 0,
    })),
...
export function getMockDistributions(
  ...
  if (month === undefined) {
    const yearData =
      type === 'expense'
        ? yearlyExpenseTotals[String(year)]
        : yearlyIncomeTotals[String(year)]
    if (!yearData) return []
    ...
    const total = Array.from(tagTotals.values()).reduce(
      (sum, t) => sum + t.amount,
      0,
    )
    if (total === 0) return []
...
export function getMockMonthlyTotals(
  ...
  return (
    yearData[tagId] ??
    MONTH_NAMES.map((monthName, i) => ({ month: i, monthName, amount: 0 }))
  )
}
